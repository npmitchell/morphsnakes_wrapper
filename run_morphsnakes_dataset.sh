...
for (( num=190; num<=212; num++ )); do     
	...
	python /mnt/data/code/morphsnakes_wrapper/morphsnakes_wrapper/run_morphsnakes.py -i Time_000${idx}_c1_stab_Probabilities.h5 -init_ls $initls -o $mslsDir -prenu $pre_nu -presmooth $pre_smoothing -ofn_ply mesh_000${idx}.ply -ofn_ls msls_000${idx}.h5 -l1 $lambda1 -l2 $lambda2 -nu $nu -postnu $post_nu -smooth $smoothing -postsmooth $post_smoothing -exit $exit_thres -channel 1 -dtype h5 -ss $ssfactor -include_boundary_faces -rad0 80 -n $niter -save -adjust_for_MATLAB_indexing ; 
	# -permute zyxc
done
